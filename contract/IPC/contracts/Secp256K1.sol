...
contract Secp256k1 {
    ...
    function submod(uint256 x, uint256 y) public pure returns (uint256) {
        uint256 x_nn;
    ...
    function g1mul2(G1Point memory p, uint256 _k)
        public
        pure
        returns (G1Point memory r)
    {
        ...
        // Get back to affine
        (uint256 x, uint256 y) = toAffine(x1, y1, z1, _pp);
        return G1Point(x, y);
    }
    ...
    // 椭圆曲线上的乘法
    function ECCPub(uint256 prikey) public pure returns (uint256[2] memory) {
